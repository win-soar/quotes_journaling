ActiveAdmin.register Circle do
  ...
  index do
    ...
    column :name
    column :description
    column('参加URL') { |c| link_to c.join_token, circle_join_signup_url(join_token: c.join_token) }
  ...
  show do
    attributes_table do
      ...
      row :description
      row('参加URL') do |c|
        url = circle_join_signup_url(join_token: c.join_token)
    ...
    panel 'メンバー一覧' do
      table_for circle.users.order(created_at: :asc) do
        column :name
        column :email
